...
from analytics import kalman_filter_beta,ols_hedge_ratio, spread_and_zscore, rolling_correlation,trading_signal
...
if "price_history" not in st.session_state:
    st.session_state.price_history = {}  # { "BTCUSDT": [prices...] }
...
def compute_pair_metrics(left, right, window=50):
    ...
    if len(histL) < window or len(histR) < window:
        return (
...
# --- Store history for ADF ---
for t in st.session_state.buffer[-new_ticks:]:
    sym = t["symbol"].upper()
    price = float(t["price"])

    if sym not in st.session_state.price_history:
        st.session_state.price_history[sym] = []

    st.session_state.price_history[sym].append(price)

    # Keep rolling window
    MAX_HISTORY = 2000
    if len(st.session_state.price_history[sym]) > MAX_HISTORY:
        st.session_state.price_history[sym] = st.session_state.price_history[sym][-MAX_HISTORY:]
...
header_ph.title(f"Realtime — {page}")

if page == "Graphs":
    st.subheader("Charts & Volume")
    syms_list = [s.strip().upper() for s in symbols.split(",") if s.strip()]
    if not syms_list:
        ...
        for i, tab in enumerate(tabs):
            ...
            with tab:
                ...
                MAX_CANDLES = 200
                ohlcv_plot = ohlcv.tail(MAX_CANDLES)

                # ---- Generate Candle + Volume ----
                ...
                line_fig.add_trace(go.Scatter(
                    ...
                    y=ohlcv_plot['close'],
                    mode='lines',
                    name=f"{sym} Close Price"
                ))
                line_fig.update_layout(
                    title=f"{sym} Price Line Chart",
                    xaxis_title="Time",
                    yaxis_title="Price",
                    height=350
                )
        ...
        if len(syms_list) >= 2:
            ...
            spread, zscore, corr, beta, _, adf_res = compute_pair_metrics(left_sym, right_sym, window=50)
            
            p_col1, p_col2, p_col3 = st.columns(3)
            p_2col1, p_2col2, p_2col3 = st.columns(3)
    ...
    st.subheader("Live Statistics Data")
    ...
    if len(syms_list) >= 2:
        left_sym, right_sym = syms_list[0], syms_list[1]
        spread, zscore, corr, beta, beta_series, adf_res = compute_pair_metrics(left_sym, right_sym, window=50)
        last_spread = float(spread.iloc[-1]) if not spread.empty else None
        last_z = float(zscore.iloc[-1]) if not zscore.empty else None
        last_corr = float(corr.iloc[-1]) if not corr.empty else None
        signal = trading_signal(last_z)
        ...
        pa_c3 = st.metric("Rolling Corr (50s)", 
                  f"{last_corr:.4f}" if last_corr is not None else "N/A",
                  delta="High" if last_corr > 0.7 else ("Low" if last_corr < 0.3 else "Neutral"))
        pa_c4 = st.metric("Hedge Ratio β", f"{beta:.4f}" if beta is not None else "N/A")
